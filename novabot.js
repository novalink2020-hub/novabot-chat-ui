// =========================================================
// NovaBot v7.5 – App Chat Logic
// Fullscreen Chat – Connected to NovaBot Brain
// =========================================================

window.addEventListener("load", () => {
  const NOVA_CONFIG = {
    API_PRIMARY: "https://novabot-brain.onrender.com",
    SOUND_URL:
    ...
    STORAGE_KEY: "novabot_v7.5_conversation",
  ...
  const chatBody = document.getElementById("novaChatBody");
  ...
  function playNovaSound() {
    if (!NOVA_CONFIG.SOUND_URL) return;
    if (soundCount >= 3) return;
    try {
      const a = new Audio(NOVA_CONFIG.SOUND_URL);
      a.play().catch(() => {});
      soundCount++;
    } catch (e) {}
  }
  ...
  async function callNovaApi(message) {
    try {
      const response = await fetch(NOVA_CONFIG.API_PRIMARY, {
        method: "POST",
        headers: {
  ...
  function createSubscribeCard(type) {
    ...
    btnSecondary.addEventListener("click", () => {
      const url = isBusiness ? NOVA_CONFIG.SERVICES_URL : NOVA_CONFIG.SUBSCRIBE_URL;
    ...
    btnPrimary.addEventListener("click", async () => {
      ...
      if (NOVA_CONFIG.FEEDBACK_API) {
        try {
          await fetch(NOVA_CONFIG.FEEDBACK_API, {
            method: "POST",
  ...
  function createBotLeadCard() {
    ...
    btn.addEventListener("click", () => {
      ...
      if (NOVA_CONFIG.FEEDBACK_API) {
        try {
          fetch(NOVA_CONFIG.FEEDBACK_API, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
              type: "lead",
      ...
      window.location.href = `mailto:${NOVA_CONFIG.CONTACT_EMAIL}?subject=${subject}&body=${body}`;
    });
  ...
  function saveConversation() {
    try {
      ...
      localStorage.setItem(NOVA_CONFIG.STORAGE_KEY, JSON.stringify(payload));
    } catch (e) {}
  }

  function restoreConversationIfFresh() {
    try {
      const raw = localStorage.getItem(NOVA_CONFIG.STORAGE_KEY);
      if (!raw) return;
      const data = JSON.parse(raw);
      if (!data || !data.ts || !Array.isArray(data.history)) return;
      if (Date.now() - data.ts > NOVA_CONFIG.STORAGE_TTL_MS) {
        localStorage.removeItem(NOVA_CONFIG.STORAGE_KEY);
        return;
      }
  ...
  async function handleSend() {
    ...
    if (result && result.ok && result.reply) {
      ...
      replyText =
        ...
        "إذا لم يصلك رد متكامل، يمكنك دائمًا العودة لمقالات نوفا لينك للحصول على أفكار عملية إضافية.";
    }
